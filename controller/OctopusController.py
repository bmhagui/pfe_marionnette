...
class controller(Sofa.PythonScriptController):
    ...
    def scaleUpBVHAnimation(self):

        q = [0., 0., -0.7, 0.7]

        positions = self.node.getChild('trajectory').getChild('dof').getObject('dof').findData("position").value
        for i in range(0,35):
            positions[i][0] *= 100
            positions[i][1] *= 100
            positions[i][2] *= 100 
            positions[i][2] += 30
            rotate(positions[i],q)
            positions[i][0] += 10

        positionsStr = ""
        for i in range(0,35):
            if i in [4, 5, 6, 11, 15, 16, 17, 21, 22, 23, 26, 27, 31, 32]:
                positionsStr += str(positions[i][0]) + " " + str(positions[i][1]) + " " + str(positions[i][2]) + " " 
